...
public class MessageCenterActivity extends BaseActivity {
    ...
    final String[] tabs = new String[]{"消息", "活动", "审批"};
    ...
    private void initView() {
        ...
        fragments.add(ActiveFragment.newInstance());
        // 判断是否为管理员和已经加入社团
        int UserType = getIntent().getIntExtra(UserConstants.USER_TYPE, UserConstants.UserType.Ordinary_Users);
        int assoId = SPUtils.getInstance().getInt(SharePrefer.associationId, 0);
        if (UserType != UserConstants.UserType.Ordinary_Users && assoId != 0) {
            fragments.add(ApprovalFragment.newInstance());
        }
